...
async def deliver_message(
    ...
    async with async_session() as session:
        sender = (
            await session.execute(select(User).where(User.telegram_id == sender_telegram_id))
        ).scalar_one_or_none()
        receiver = (
            await session.execute(select(User).where(User.id == receiver_user_id))
        ).scalar_one_or_none()

        if sender is None or receiver is None:
            logger.warning(f"deliver_message: sender or receiver not found "
    ...
    sender_handle = f"@{sender.username}" if sender.username else "без username"

    notification = (
        f"💌 <b>Новое сообщение от игрока</b>\n\n"
        f"👤 {sender_name} ({sender_handle})\n\n"
        f"<i>{text}</i>\n\n"
        f"Чтобы ответить — напиши в чат, но помни: пока это односторонняя доставка."
    )

    try:
        await bot.send_message(receiver.telegram_id, notification)
        logger.info(f"Message delivered: {sender.id} → {receiver.id}")
        return True
