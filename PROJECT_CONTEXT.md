...
## 22. ЭТАП 3 — КТО ПОСМОТРЕЛ ПРОФИЛЬ (2026-09-24)

### Кратко

Два режима уведомления о просмотрах профиля:

1. **Реальные просмотры.** Логируются, когда юзер:
   - видит чужой профиль в поиске (`matching`),
   - сравнивает себя с другом (`compare`).
   Юзеру приходит: «Сегодня **N человек** зашли в твой профиль» (без имён).

2. **Псевдо-просмотры.** Для юзеров, которых не было **4+ дней**.
   Юзеру приходит: «Твой вайб заинтересовал **N игроков** 🔥 ⚡ 🌙» —
   чтобы вернулся.

**Приватность:** имена не показываем никогда. Только количество и «типы» игроков
(эмодзи-архетипы: 🔥, ❄️, ⚡, 🌙, ✨, 🎯, 🎭, 🧠, 💥, 🎨).
...
| Файл | Назначение |
|---|---|
| `services/social/profile_views.py` | Лог + агрегаты + псевдо + форматирование |
| `services/notifications/profile_views.py` | Планировщик `profile_views_loop` |

### Изменённые файлы

| Файл | Что |
|---|---|
| `bot/handlers/compare.py` | `log_view` после сравнения с другом / игроком |
| `bot/handlers/matching.py` | `log_view` после показа кандидата |
| `main.py` | Запуск `profile_views_loop` |
| `services/analytics/tracker.py` | Событие `profile_viewed` (дополнено) |
...
┌─ Реальные просмотры ─────────────────────────────┐
│                                                   │
│  matching: показ кандидата                       │
│     ↓                                             │
│  log_view(viewer=me.id, viewed=c.id, "matching") │
│                                                   │
│  compare: сравнение                              │
│     ↓                                             │
│  log_view(me.id → friend.id, "compare")          │
│  log_view(friend.id → me.id, "compare")          │
│                                                   │
│  Таблица: profile_views(viewer_id, viewed_id,    │
│                          source, created_at)     │
│                                                   │
└───────────────────────────────────────────────────┘
...
│                                                   │
│  18:00 по TZ юзера?                              │
│     ├── реальные просмотры за 7 дней?           │
│     │   → да: «N человек зашли» (priority=3)    │
│     │   → архетипы: топ-3 (анонимно)            │
│     │                                            │
│     └── юзера не было 4+ дней?                  │
│         → да: псевдо «Твой вайб заинтересовал»   │
│         (priority=4)                             │
│                                                   │
│  → schedule_notification(kind="profile_views")  │
│                                                   │
└───────────────────────────────────────────────────┘
```
...
| Где | Что | Значение |
|---|---|---|
| `services/social/profile_views.py` | `MIN_DAYS_AWAY_FOR_PSEUDO` | 4 |
| `services/social/profile_views.py` | `REAL_VIEWS_WINDOW_DAYS` | 7 |
| `services/notifications/profile_views.py` | `TARGET_HOUR` | 18 |
| `services/notifications/profile_views.py` | `CHECK_INTERVAL_SECONDS` | 3600 (1 час) |

### Псевдо-логика

| Давность отсутствия | Кол-во «игроков» |
|---|---|
| 4-6 дней | 1-2 |
| 7-13 дней | 2-3 |
| 14+ дней | 3-5 |

Типы игроков (эмодзи): 🔥 ❄️ ⚡ 🌙 ✨ 🎯 🎭 🧠 💥 🎨 — случайная выборка.

### Приоритет

- **Реальные просмотры:** `priority=3` — если дневной лимит набран, всё равно
  пройдёт (полезно — юзер знает, что кто-то зашёл).
- **Псевдо:** `priority=4` — если лимит набран, отбрасывается. Правильно:
  псевдо — самое «слабое» из уведомлений.

### События

| Событие | Когда | Payload |
|---|---|---|
| `profile_viewed` | Залогирован просмотр (viewer → viewed) | `{source}` |
| `profile_views_sent` | Hub отправил уведомление | `{kind}` |

### Как тестировать

**Логирование:**
1. Зайди под двумя аккаунтами (А и Б).
2. А → `🎯 Найти игроков` → увидь профиль Б.
3. БД: `SELECT * FROM profile_views ORDER BY id DESC LIMIT 5;` — должны быть записи.

**Псевдо-рассылку:**
1. В `services/notifications/profile_views.py` временно поставь `TARGET_HOUR` = текущий час.
2. Если у тебя `last_active_at` больше 4 дней назад → должно уйти псевдо.
3. Если нет — в БД обнови `UPDATE users SET last_active_at = NOW() - INTERVAL '5 days' WHERE telegram_id = ...;`
4. Redeploy → жди до 1 часа → в логах `[VIEWS] queued user=...`.
5. Hub отправит (до 15 минут).
6. **Откати** `TARGET_HOUR` на 18.

### History проблем

| Проблема | Решение |
|---|---|
| Юзер хочет знать «кто», но это creepy | Никогда не пишем имена, только количество + архетипы |
| Пустые просмотры у неактивных | Псевдо-логика: 4+ дней → генерим «типы игроков» |
| Слишком частые уведомления | Hub дневной лимит + `_already_sent_today` |
| Логирование тормозит основной поток | `log_view` вызывается **после** `answer`, обёрнут в try/except |
| Дубли при сравнении с другом | Логируем **обоих**, но `viewer != viewed` в `log_view` |

### Точка отката

- **Отключить псевдо:** в `services/social/profile_views.py` закомментировать
  вызов `get_pseudo_views(...)`.
- **Отключить всё:** в `services/notifications/profile_views.py` закомментировать
...
## 25. ПЛАН РАЗВИТИЯ — ФЛАГИ И ЗАПЛАНИРОВАННЫЕ ФИЧИ (2026-09-24)

### 🔔 Feature flags — что добавить

**Где:** `services/feature_flags.py` → словарь `DEFAULTS`.

**Зачем:** сейчас hub использует эти флаги с `default=True`, но в БД их
нет — значит, нельзя выключить через `/admin → ⚙️ Feature flags`, и в
админке они не видны.

**Что добавить (7 флагов):**
...
DEFAULTS: dict[str, bool] = {
    ...
    # === Новые (Этап 1-4) ===
    "horoscope_enabled": True,          # 🔮 Гороскоп
    "secret_feature_enabled": True,     # 💡 Секретные фичи
    "profile_views_enabled": True,      # 👀 Просмотры профиля
    "weekly_vibe_enabled": True,        # 🗓 Вайб-отчёт
    "tops_enabled": True,               # 🏆 Топы
    "premium_reminder_enabled": True,   # 💎 PRO-напоминания
    "chat_reminder_enabled": True,      # 💬 Chat reminder
}
```

**Что даст:**
- Все фичи появятся в `/admin → ⚙️ Feature flags`.
- Мгновенное выключение без деплоя (снять галочку).
- Единый источник правды по всем уведомлениям.

**Приоритет:** низкий. **Стоимость:** 1 файл, 2 минуты.

---

### 🎯 Запланированные фичи (по приоритету)

#### 🥇 Средний приоритет

| Фича | Что | Объём |
|---|---|---|
| **Кланы / команды** | Юзеры объединяются в кланы (5-10 чел). Клан соревнуется с другими по очкам. Общий чат клана, общий рейтинг, клановые квесты. | Большой |
| **Взаимные лайки** | Юзер ставит лайк профилю из матчинга. Если второй ответил взаимно → «мэтч» + чат. Расширение матчинга. | Средний |
| **Дуэли вайбов** | PvP 1-на-1. Юзер жмёт «⚔️ Дуэль» → бот находит случайного игрока → AI сравнивает вайбы → победитель получает очки. | Средний |

#### 🥈 Низкий приоритет

| Фича | Что | Объём |
|---|---|---|
| **Вайб-магазин** | Трата очков на: темы карточек, boost ×2 очков на день, особые архетипы, стикеры. Даёт очкам смысл. | Средний |
| **Автопродление PRO** | Рекуррентные платежи YooKassa. Отложено с раздела 15. | Большой |
| **Монетизация пакетами / подарками** | Пакеты анализов, «подарить PRO другу». Отложено с раздела 15. | Средний |
| **A/B-тесты промтов — автопобедитель** | Автоматически выбирать выигрышный вариант промта по share rate. | Средний |

#### 🥉 Мелочи (можно делать в любой момент)

| Мелочь | Что |
|---|---|
| **Точные `points_gained` за неделю** | Сейчас в недельном вайб-отчёте `points_gained = 0`. Нужна история начисления очков. |
| **Свои иконки для карточки «Вайб-месяц»** | Сейчас используются «чужие» (`charisma` для «Анализов»). Можно нарисовать свои через `tools/build_embedded_icons.py`. |
| **Дополнительные сезоны** | Сейчас есть Хэллоуин, НГ, 8 марта, 23 февраля. Можно добавить: 1 сентября, день космонавтики, и т.д. |
| **Rare-архетипы расширить** | Сейчас 8 легендарных. Можно добавить редкость «эпический» между common и legendary. |

---

### 📊 Что уже готово (не забыть)

| Этап | Что |
|---|---|
| **MVP** | Фото → AI → карточка → share |
| **Виральность** | Рефералка, приглашения |
| **Социальная игра** | Поиск, матчи, чат, AI-помощник |
| **PRO** | Тарифы + напоминания |
| **Админка** | 12+ разделов |
| **Шаг 1.2** | Легендарные архетипы |
| **Шаг 1.3** | Вайб-отчёт (по запросу + недельный) |
| **Этап 1** | Инфраструктура уведомлений (hub, тумблеры) |
| **Этап 2** | Карма дня, Секретная фича, Гороскоп |
| **Этап 3** | Совместимость со звёздами, Просмотры профиля |
| **Этап 4** | Рефакторинг loops на hub |

---

### 🗺️ Порядок работы (предложение)

Если хочешь новый функционал, а не мелочи:

1. **Флаги** (5 минут) — добавить 7 флагов в `DEFAULTS`.
2. **Взаимные лайки** — усиление матчинга, средний объём.
3. **Дуэли вайбов** — PvP, даёт быстрый вовлекающий эффект.
4. **Кланы** — крупная фича, делать на «сытый» продукт.
5. **Вайб-магазин** — даёт смысл очкам.

**Если хочется «вау» быстро** → Дуэли вайбов.
**Если хочется долгосрочного удержания** → Кланы.
**Если хочется углубить текущую механику** → Взаимные лайки.

---

### ⚠️ Что важно помнить

1. **Файлы с Python-кодом — не в `data/`.** Была проблема на BotHost.
   Списки/константы кладём прямо в модуль сервиса.
2. **Все уведомления — через hub.** Не плодим новые `*_loop` без hub.
3. **Feature flag для каждой новой фичи.** Чтобы можно было выключить без деплоя.
4. **Приоритет в hub:** 1 — критично (premium), 4 — можно пропустить (tops).
5. **Тестировать на BotHost, не локально** — база доступна только изнутри.

---

## 26. ЭТАП 5 — ОТЧЁТЫ ПО КАМПАНИЯМ + РЕДАКТОР + ВОЗОБНОВЛЕНИЕ (2026-09-25)

### Кратко

Доработали админ-раздел «📣 Подписки»:

1. **📋 Подписчики** — список юзеров с username и датой.
2. **📊 Разбивка по дням** — сколько подписок в каждый день + стоимость.
3. **📤 Экспорт CSV** — файл для отчёта заказчику.
4. **📊 Сводка по всем кампаниям** — общий счётчик + общая сумма.
5. **✏️ Редактор** — изменение цены / лимита / бюджета.
6. **▶️ Возобновление** — раньше можно было только остановить.

### Новые файлы

| Файл | Назначение |
|---|---|
| `services/subscriptions/reports.py` | Расчёты + форматирование + CSV |

### Изменённые файлы

| Файл | Что |
|---|---|
| `bot/keyboards/admin.py` | Новые клавиатуры: `subs_campaign_card_kb` (с ⏸/▶️), `subs_edit_kb`, `subs_edit_cancel_kb`, `subs_subscribers_kb` |
| `admin/handlers.py` | Хендлеры: `subs_card_*`, `subs_subs_*`, `subs_daily_*`, `subs_export_*`, `subs_stop_*`, `subs_resume_*`, `subs_edit_*`, `subs_summary`, `subs_edit_input` (FSM) |
| `admin/subscriptions_wizard.py` | Новые шаги: цена → лимит → бюджет → подтверждение |
...
💰 Стоимость: 1305 ₽
   Бюджет: 5000 ₽ (исп. 26%)
👥 Лимит подписчиков: 87/1000 (8.7%)

[📋 Подписчики]  [📊 Разбивка по дням]
[📤 Экспорт CSV]  [✏️ Изменить]
[🔄 Обновить]  [⏸ Остановить]  [⬅️ К списку]
```

**Подменю редактирования (`✏️ Изменить`):**
- 💰 Изменить цену
- 👥 Изменить лимит
- 💵 Изменить бюджет

**FSM:** юзер жмёт → бот просит число → юзер вводит → сохраняется → показывается карточка с новыми значениями.

### Создание кампании (обновлено)

Порядок шагов:
1. Ссылка / `@username`.
2. Название (авто или вручную).
3. **💰 Цена за подписчика** (новое).
4. **👥 Лимит подписчиков** (новое).
5. **💵 Бюджет** (новое).
6. Подтверждение со сводкой.

**Раньше** были хардкод-дефолты `price=2.0, limit=1000, budget=2000`.
**Теперь** все значения вводит админ.

### Возобновление кампании

- **⏸ Остановить** → `is_active=False`, `status="stopped"`, `ended_at=now`.
- **▶️ Запустить** → `is_active=True`, `status="active"`, `ended_at=None`.
- **`started_at` НЕ трогаем** — сохраняем первое включение для истории.

### API сервиса `reports.py`

| Функция | Что возвращает |
|---|---|
| `get_campaign_stats(campaign_id)` | Dict с 5 блоками: campaign, counts, money, limits, period |
| `get_campaign_subscribers(campaign_id, limit, offset, only_confirmed)` | Список юзеров |
| `get_campaign_daily_breakdown(campaign_id)` | Разбивка по датам |
| `format_campaign_card(stats)` | Текст карточки |
| `format_subscribers_list(subscribers, total, page, per_page)` | Текст списка |
| `format_daily_breakdown(breakdown, campaign_name, price)` | Текст разбивки |
| `export_to_csv(campaign_id, only_confirmed)` | Байты CSV (utf-8-sig) |

### Формулы

- **Стоимость:** `confirmed × price_per_subscription`
- **Бюджет использован:** `(total_cost / budget) × 100%`
- **Лимит использован:** `(confirmed / subscriber_limit) × 100%`
- **Ожидает:** `total - confirmed` (всего событий минус подтверждённые)

### CSV-экспорт

Колонки:
```
user_id, telegram_id, username, first_name, status, checked_at, confirmed_at
```

Кодировка `utf-8-sig` (BOM) — **Excel откроет кириллицу без проблем**.
Имя файла: `campaign_<id>_YYYY-MM-DD.csv`.

### Приватные каналы

**Можно ли ставить приватный канал в кампанию?**
- Да, кампания создаётся, но как **черновик** (`status="draft"`, `is_active=False`).
- Проверка подписки **не работает** — Telegram API не умеет проверять по invite-ссылке.
- Юзеры **не видят** эту кампанию в оффере обязательной подписки.
- **Решение:** сделать канал публичным (задать `@username` в настройках) — тогда всё заработает.

### History проблем

| Проблема | Решение |
|---|---|
| Раньше кампанию нельзя было запустить обратно | Добавили `subs_resume_<id>` |
| Раньше цена и лимит были хардкод | Добавили шаги в FSM мастера + редактор |
| `admin_input` перехватывал числа от FSM | `subs_edit_input` идёт **до** `admin_input` в файле |
| CSV не открывался в Excel с кириллицей | `utf-8-sig` (BOM) |
| Фильтр `subs_edit_*` ловил и подкнопки | `& ~F.data.startswith(...)` для исключения |
| Детальные detached instances при сборе статистики | Снимок полей до выхода из сессии |

### Точка отката

- **Отключить редактор:** закомментировать кнопку `✏️ Изменить` в `subs_campaign_card_kb`.
- **Вернуть дефолты в мастере:** в `subscriptions_wizard.py` в `cb_confirm` заменить `price=price, subscriber_limit=limit, budget=budget` на `price=2.0, subscriber_limit=1000, budget=2000`.
- **Убрать возобновление:** в карточке закомментировать кнопку `▶️ Запустить`.

---

## 27. ПЛАНЫ ПО ПРИВАТНОСТИ И ОБРАБОТКЕ ФОТО (отложено)

### Что сделано (2026-09-25)

В `/start` добавлена строка:
```
📸 Отправляя фото, ты соглашаешься на его обработку через AI-сервис
GigaChat. Фото не хранится на наших серверах и не показывается
другим юзерам.
```

### Как сейчас работает с фото

| Где | Что хранится |
|---|---|
| **BotHost (сервер бота)** | Ничего — только код |
| **Postgres (БД)** | `telegram_file_id`, `telegram_file_unique_id`, `analysis_json` |
| **Telegram** | Само фото, ~1-2 дня (пока не перешлёшь) |
| **GigaChat** | Фото на время анализа (по их политике) |

**Самого фото на сервере бота нет.** В БД — только **ссылка** (`file_id`) и **результат AI** (`analysis_json`).

### Что запланировано (отложено)

#### 1. `/privacy` — команда с политикой обработки

Полный текст:
- что собираем,
- куда отправляем (GigaChat),
- как долго храним,
- как удалить.

**Приоритет:** средний (152-ФЗ «О персональных данных»).

#### 2. Уведомление при первом анализе

Первое фото → бот пишет:
> 📸 Отправляю фото в AI для анализа. Фото не сохраняется
> у нас и не показывается другим. Продолжить?
> [✅ Понятно]

С кнопкой подтверждения. Сохраняется флаг `user.privacy_accepted_at` (нужна миграция).

**Приоритет:** низкий (можно и без подтверждения).

#### 3. `/delete_my_data` — удаление истории

Юзер жмёт → бот удаляет:
- `photo_analyses` (все его),
- `profiles`,
- `user_tests`,
- (опционально) `messages`, `matches`, `chats`.

С подтверждением.

**Зачем:** по 152-ФЗ юзер имеет право требовать удаления своих данных.

**Приоритет:** низкий (но полезно для «вау» — «мы уважаем твои данные»).

#### 4. Автоудаление `telegram_file_id`

Через N дней (например, 7) после анализа — затирать `telegram_file_id = NULL`.
Файл всё равно протухнет в Telegram, а `analysis_json` останется.

**Минус:** нельзя будет «показать исходное фото».

**Приоритет:** низкий.

### Важно помнить

- GigaChat — **внешний сервис**, его политику мы не контролируем.
- Telegram хранит фото временно, но **file_id можно запросить**, если файл ещё жив.
- Если хочешь максимум приватности — затирай `telegram_file_id` сразу после анализа.