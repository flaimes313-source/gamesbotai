...
async def _send_referrer_card(message: Message, referrer_user_id: int) -> None:
    ...
    caption = (
        ...
        f"Посмотри, какой у него вайб 👇\n\n"
        f"🧨 <b>{profile.archetype}</b>\n\n"
        f"А ты какой? Отправь фото — сравним! 😏"
    )
...
async def cmd_start(message: Message):
    ...
    if referrer_id:
        ...
        "📸 <i>Отправляя фото, ты соглашаешься на его обработку "
        "через AI-сервис GigaChat. Фото не хранится на наших серверах "
        "и не показывается другим юзерам.</i>\n\n"
        "Ну что, какой ты? 👀"
    )
